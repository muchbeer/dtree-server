...
export const getBalance = tryCatch(async (req, res) => {
    ...
    const retrieve_last_record = await getLastRecord();
...
export const topupAirtime = tryCatch(async (req, res) => {
  ...
  const { username, topup } = req.body; 
      ...
      const current_balance_object = await getLastRecord();
      ...
      const sql_new_balance = 'INSERT INTO dtree_airtime_balance ( balance, topup, user_email, balance_spent ) VALUES ( $1, $2, $3, $4 ) RETURNING *';
      await connect.query( sql_new_balance, values_balance_updated )
              .then((topup_result) => {
